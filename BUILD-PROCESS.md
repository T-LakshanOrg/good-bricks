...
- Lay out the editor. Our self-hosted CMS adds three options that stock Pages CMS does not have: `position: sidebar` puts a field in a sticky right column, `width: half` sits two neighbouring fields side by side, and `collapsible: true` folds a group of fields under a heading. Use them for the fields an editor reaches for most, such as status, price and the agent. These options only work on our CMS. The hosted pagescms.org would refuse to load a config that contains them, so if a site ever moves off our CMS they must be removed first. An agent working on our CMS should not remove them as invalid.
...
- Build a page listing all properties, newest first, with a row of buttons at the top that lets a visitor narrow the list by status and by kind of property. Because every property is already on the page, the buttons only need to hide and show what is there, so the filtering happens instantly in the browser and the site is still made of plain files. Only offer a button for something that is actually in the list, and tell the visitor how many properties they are looking at.
...
- Follow a clear design brief. The one used here was: clean, modern and almost entirely black, white and grey, in the manner of a good estate agency – white pages with a very slightly darker grey for alternating bands, near-black text, mid-grey for supporting lines, hairline grey rules, square corners on everything, generous space, and content laid out in bordered cards on a grid. The only colour on the site is the listing status on a property: green for sale, amber under offer, grey for sold, black to let. Everything else earns attention through size, weight and space rather than colour. Ask for a pairing of two web fonts, one for headings and one for body text, with ordinary system fonts named as a fallback so the page still reads properly if the fonts are slow to arrive.
